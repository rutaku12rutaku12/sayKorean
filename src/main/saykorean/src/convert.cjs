...
function convertToJson(entries) {
    const json = {};
    // 대충 다 허용하는 정규식
    const matches = entries.match(/Map\.entry\s*\(\s*["']([^"']+)["']\s*,\s*["']([^"']+)["']\s*\)/gs);
...
// JSON 파일 생성
fs.writeFileSync('ko.json', JSON.stringify(convertToJson(koEntries), null, 2));
fs.writeFileSync('ja.json', JSON.stringify(convertToJson(jaEntries), null, 2));
fs.writeFileSync('zh-Cn.json', JSON.stringify(convertToJson(cnEntries), null, 2));
fs.writeFileSync('en.json', JSON.stringify(convertToJson(enEntries), null, 2));
fs.writeFileSync('es.json', JSON.stringify(convertToJson(esEntries), null, 2));

console.log('✅ 다국어 생성 완료!');