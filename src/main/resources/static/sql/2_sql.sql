...
CREATE TABLE IF NOT EXISTS genre (                                       -- 장르 테이블 생성
  genreNo   INT NOT NULL AUTO_INCREMENT,                        -- PK: 양수 자동증가
...
CREATE TABLE IF NOT EXISTS users (                                      -- users 테이블 생성(백틱 사용)
  userNo       INT   NOT NULL AUTO_INCREMENT,                 -- PK: 사용자번호 자동증가
...
CREATE TABLE IF NOT EXISTS attendance (                                  -- attendance 테이블 생성
  ...
  -- [시험 테이블 생성]
CREATE TABLE test (
testNo INT NOT NULL AUTO_INCREMENT, -- 시험번호 (PK) 
testTitle VARCHAR(50) NOT NULL, -- 시험명 
studyNo INT NOT NULL, -- 교육번호 (FK) 
PRIMARY KEY (testNo), 
...
CREATE TABLE testItem (
  testItemNo INT NOT NULL AUTO_INCREMENT,  -- PK: 시험문항번호
  question LONGTEXT NOT NULL,              -- 질문
  examNo INT NOT NULL,                     -- 예문번호
  testNo INT NOT NULL,                     -- 시험번호
...
CREATE TABLE ranking (
  rankNo INT NOT NULL AUTO_INCREMENT,  -- PK: 랭킹번호
  testRound INT NOT NULL,              -- 시험회차
  userAnswer VARCHAR(500) NOT NULL DEFAULT '객관식 문항이거나 공란으로 제출했습니다.', -- 사용자응답
  isCorrect TINYINT NOT NULL DEFAULT 0,  -- 정답여부 (0: 오답, 1: 정답)
  resultDate DATETIME NOT NULL DEFAULT NOW(),  -- 제출일시
  testItemNo INT NOT NULL,              -- FK: 시험문항번호
  userNo INT NOT NULL,                  -- FK: 사용자번호
  PRIMARY KEY (rankNo),
 ...
 loadNo INT NOT NULL AUTO_INCREMENT, -- 로딩번호 (PK) 
 loadTitle TEXT NOT NULL, -- 로딩제목 
 loadInfo TEXT NOT NULL, -- 로딩이미지 설명 
 loadFileName TEXT NOT NULL, -- 로딩이미지 파일명 
 PRIMARY KEY (loadNo) 
 );
